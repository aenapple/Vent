...
class UIServer(object):
    ...
    def serv(self, use_global_hooks=True):
        if use_global_hooks:
            keyboard.hook_key('t', self.on_key_t)
            keyboard.hook_key('l', self.on_key)
            keyboard.hook_key('e', self.on_key)
                ...
                if r:
                    ...
                    if not use_global_hooks:
                        if msvcrt.kbhit():
                            c = msvcrt.getch()
                            if c == 't':
                                self.on_key_t(MyKeyboardEvent(c, 'down'))
                            else:#elif c in ('e', 'l'):
