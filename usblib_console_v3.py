...
backend = usb.backend.libusb0.get_backend(find_library=lambda x: "libusb-1.0.dll")
...
def SelectByProductString(devices, product):
    for device in devices:
        ...
        Product = usb.util.get_string(handle, device.iProduct)
...
class LXB(object):
    def __init__(self, device):
        ...
        #interface = cfg[(0, 0)]
        # aen
        interface = device.configurations()[0].interfaces()[1]
        ...
        for endp in self.interface.endpoints():
            ...
            if endp.bEndpointAddress & 0x80:
                self.endin = endp
                print('IN set to ', hex(endp.bEndpointAddress))
                print(dir(endp))
                print(endp.wMaxPacketSize)
            else:
                self.endout = endp
                print('OUT set to ', hex(endp.bEndpointAddress))
    ...
    def Write(self, data, timeout=100):
        ...
        return self.handle.write(self.endout.bEndpointAddress, data, timeout)
    ...
    def Read(self, count=None, timeout=100):
        try:
            ...
            return self.handle.read(self.endin.bEndpointAddress, count or self.endin.wMaxPacketSize, timeout)
